...
const startString = `<template v-if="viewState === 'setup'">`;
const endString = `<template v-else-if="viewState === 'progress'">`;
...
const replacement = `<template v-if="viewState === 'setup'">
        <!-- Header -->
...
// wait! Notice I am including the endString `<template v-else-if="viewState === 'progress'">` in my replacement? I should not chop it off.
// Let's modify the newContent assembly to leave endString.
const newContent = content.substring(0, startIndex) + replacement + content.substring(endIndex + endString.length);
